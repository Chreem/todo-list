...
class RootSize {
    constructor(config) {
        const {win, count, maxWidth} = config;
    ...
    readSize() {
         ...
         */
        if (checkOrientation()) {
            width = this.win.innerWidth || this.docHtml.clientWidth;
         ...
         */
            ...
            width = this.win.innerHeight || this.docHtml.clientHeight;
            if (!width && document.body) {
    ...
    getEmSize(px) {
        return Math.floor(px / this.fontSize * 100) / 100
    }
}

export default new RootSize({
    win: window,
});
